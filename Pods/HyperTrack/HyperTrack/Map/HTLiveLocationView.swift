...
class HTLiveLocationView: HTCommonView {
            ...
            for userId in userIds {
                if (!HTGenericUtils.isCurrentUser(userId: userId)){
                    if let act = HTConsumerClient.sharedInstance.getUser(userId:userId)?.actions?.last {
                        ...
                        let user = HTConsumerClient.sharedInstance.getUser(userId: userIds.first!)
                        self.processTimeAwarePolyline(userId: userIds.first!, timeAwarePolyline:user?.expandedUser?.timeAwarePolyline, disableHeroMarkerRotation: false)
                    }
                }
            }
        }
    ...
    @IBAction func startTracking(_ sender: Any) {
        ...
        self.interactionViewDelegate?.didTapStartLiveLocationSharing?(place: self.selectedLocation!)
        
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(didStartTracking(notification:)),
                                               name: NSNotification.Name(rawValue: HTConstants.HTTrackingStartedForLookUpId), object: nil)
    ...
    func onStartTracking(action : HyperTrackAction , userIds : [String]){
        ...
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(didEnteredMonitorRegion(notification:)),
        ...
        if let place = self.selectedLocation{
            ...
            Transmitter.sharedInstance.locationManager.startMonitorForPlace(place: self.selectedLocation!)
        }
    }
    ...
    func didEnteredMonitorRegion(notification : Notification){
        ...
        if(currentViewState == LiveLocationViewState.TRACKING_STARTED){
            if let region = notification.userInfo?["region"] as? CLRegion{
                if(selectedLocation?.getIdentifier() == region.identifier){
                    if(self.destinationAnnotation?.action != nil ){
                        if let user = self.destinationAnnotation?.action?.user{
                            if (HTGenericUtils.isCurrentUser(userId: user.id)){
                                currentViewState = LiveLocationViewState.TRACKING_COMPLETED
                                HTConsumerClient.sharedInstance.eventDelegate?.didEnterMonitoredDestinationRegionForAction!(forAction: (self.destinationAnnotation?.action)!)
    ...
    //    func showSingleUserTripSummary(userId : String){
    //
    //        let user = HTConsumerClient.sharedInstance.getUser(userId: userId)
    //        if let  action = user?.actions?.last {
    //            let statusInfo = HTStatusCardInfo.getUserInfo(action!,userId, useCase: HTConstants.UseCases.TYPE_SINGLE_USER_SINGLE_ACTION, isCurrentUser: HTGenericUtils.isCurrentUser(userId: userId))
    //           // self.statusCard?.addCompletedView()
    //        }
    //
    //
    //    }
